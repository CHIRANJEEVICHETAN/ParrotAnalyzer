  ...
  const fetchAttendanceData = async (month: string) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/api/employee/attendance/${month}`,
        {
        ...
        {attendanceData[format(selectedDate, 'yyyy-MM-dd')] ? (
          ...
          >
            ...
            <View className="mb-6 p-4 rounded-lg bg-opacity-50 bg-blue-500/10">
              <Text className={`text-base font-semibold mb-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                Daily Summary
              </Text>
              <View className="flex-row flex-wrap">
                <View className="w-1/2 mb-2">
                  ...
                  <Text className={`text-base font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    {attendanceData[format(selectedDate, 'yyyy-MM-dd')].shifts.length}
                  </Text>
                </View>
                <View className="w-1/2 mb-2">
                  <Text className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
