...
export default function ChangePassword() {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setModalMessage(error.response?.data?.error || 'Failed to change password');
      setErrorModalVisible(true);
    } finally {
      setChanging(false);
    }
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: isDark ? '#111827' : '#F3F4F6', paddingTop: Platform.OS === 'ios' ? 44 : StatusBar.currentHeight || 0 }}>
      <StatusBar
          ...
          <View className="p-4">
            <View className={`p-6 rounded-xl ${isDark ? 'bg-gray-800' : 'bg-white'}`} style={styles.card}>
              ...
              <View className="space-y-6">
                {/* Current Password */}
                <View>
                  <Text className={`text-base font-medium mb-2 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
                    Current Password
                  </Text>
