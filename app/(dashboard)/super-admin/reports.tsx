import { View, Text, ScrollView, TouchableOpacity, StyleSheet, Platform, StatusBar, Dimensions } from 'react-native';
            ...
            >
                ...
                >
                    {reportTypes.map((item, index) => (
                        <View
                            key={index}
                            className={`mr-4 p-4 rounded-xl ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}
                            style={styles.statCard}
                        >
                            <View className="flex-row items-center mb-2">
                                <View className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-blue-50'}`}>
                                    <Ionicons name={item.icon as any} size={24} color="#3B82F6" />
                                </View>
                                <Text className={`ml-2 ${item.trend.startsWith('+') ? 'text-green-500' : 'text-red-500'}`}>
                                    {item.trend}
                                </Text>
                            </View>
                            <Text className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                ...
                <View className="px-4 py-4">
                    <View 
                        className={`p-4 rounded-xl mb-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}
